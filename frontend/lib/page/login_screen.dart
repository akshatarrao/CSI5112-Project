...
class _LoginScreentState extends State<LoginScreen> {
  ...
  Future<String?> _loginUser(
      LoginData data, bool isCustomer, List<User> users) {
    ...
    if (bypassLogin) {
      ...
      if (bypassCustomer) {
        ...
        currentUser = User(
            ...
            password: "fake",
            userType: "merchant",
            id: 1); // ID needs to be 1 to match a merchant user in User data for integration tests
      }
    }
              ...
              ElevatedButton(
                ...
                onPressed: () {
                  Navigator.of(context).pushReplacement(FadePageRoute(
                    builder: (context) => bypassCustomer
                        ? MyHomePage(currentUser: currentUser)
                        : MerchantPage(currentUser: currentUser),
  ...
  Future<List<User>> getUsers() async {
    final Response response =
        await get(Uri.parse(Constants.baseApi + '/user/'));
